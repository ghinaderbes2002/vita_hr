...
import { Plus, Search, MoreHorizontal, Pencil, Trash2, List, CalendarDays } from "lucide-react";
...
export default function HolidaysPage() {
  ...
  const filteredHolidays = holidays.filter((h: Holiday) =>
    h.nameAr?.toLowerCase().includes(search.toLowerCase()) ||
    h.nameEn?.toLowerCase().includes(search.toLowerCase())
  );

  const handleEdit = (holiday: Holiday) => { setSelectedHoliday(holiday); setDialogOpen(true); };
  const handleDelete = (holiday: Holiday) => { setSelectedHoliday(holiday); setDeleteDialogOpen(true); };
  ...
  const getTypeBadgeVariant = (type: string) => {
    switch (type) {
      ...
      case "NATIONAL": return "secondary" as const;
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case "PUBLIC": return t("holidays.types.public");
      case "NATIONAL": return t("holidays.types.national");
      case "RELIGIOUS": return t("holidays.types.religious");
      default: return t("holidays.types.other");
    }
  };
              ...
              {isLoading ? (
                ...
                filteredHolidays.map((holiday: Holiday) => (
                  <TableRow key={holiday.id}>
                    ...
                    <TableCell>{format(new Date(holiday.date), "PPP", { locale: dateLocale })}</TableCell>
                    <TableCell>
