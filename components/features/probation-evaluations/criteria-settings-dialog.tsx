...
import {
  CriteriaTarget, CriteriaTargetSelect, EMPTY_TARGET, jobTitleIdsOf, jobTitleLabel, targetOf,
  useJobTitleOptions,
...
export function CriteriaSettingsDialog({ open, onOpenChange }: CriteriaSettingsDialogProps) {
  ...
  // Who the question is for: everyone, a set of job titles, or one employee.
  const [newTarget, setNewTarget] = useState<CriteriaTarget>(EMPTY_TARGET);
  const [editTarget, setEditTarget] = useState<CriteriaTarget>(EMPTY_TARGET);
  ...
  const jobTitleNames = (c: ProbationCriteria) => {
    const ids = jobTitleIdsOf(c);
    return ids.map(
      (id) =>
        jobTitleLabel(jobTitles.find((t) => t.id === id))
        || jobTitleLabel(c.jobTitles?.find((t) => t.id === id))
        || "مسمى وظيفي",
    );
  };
  ...
  function saveEdit() {
    ...
    updateCriteria.mutate(
      { id: editingId, data: {
        ...
        jobTitleIds: editTarget.jobTitleIds,
        targetEmployeeId: editTarget.targetEmployeeId,
      } },
    ...
    createCriteria.mutate(
      {
        ...
        ...(newTarget.jobTitleIds.length ? { jobTitleIds: newTarget.jobTitleIds } : {}),
        ...(newTarget.targetEmployeeId ? { targetEmployeeId: newTarget.targetEmployeeId } : {}),
      },
      { onSuccess: () => { setNewNameAr(""); setNewNameEn(""); setNewTarget(EMPTY_TARGET); } },
    );
  }
        ...
        <DialogContent className="sm:max-w-2xl max-h-[85dvh] overflow-y-auto">
                ...
                <div key={c.id} className={`gap-2 rounded-lg border p-2 ${editingId === c.id ? "space-y-2" : "flex items-center"}`}>
                  {editingId === c.id ? (
                    ...
                    <>
                      ...
                      {jobTitleNames(c).slice(0, 2).map((name, i) => (
                        <Badge key={i} variant="outline" className="text-[10px] shrink-0 border-blue-300 bg-blue-50 text-blue-700">
                          {name}
                        </Badge>
                      ))}
                      {jobTitleNames(c).length > 2 && (
          ...
          <div className="border-t pt-3 space-y-1.5">
            ...
            {/* A targeted question only reaches that employee, or everyone holding
                that job title. */}
            <CriteriaTargetSelect value={newTarget} onChange={setNewTarget} resetKey="new" />
            <p className="text-xs text-muted-foreground">
              السؤال المخصص يظهر فقط في تقييم الموظف المحدد أو في تقييمات موظفي المسميات الوظيفية
              المحددة، ولا يؤثر على التقييمات المنشأة سابقاً.
            </p>
          </div>
