...
export function useCreateJobTitle() {
  ...
  return useMutation({
    ...
    onError: (error: any) => {
      ...
      if (code === "JOB_TITLE_CODE_EXISTS") return toast.error("هذا الكود موجود مسبقاً، اختر كوداً مختلفاً");
      toast.error(error.response?.data?.error?.message || error.response?.data?.message || "حدث خطأ أثناء إنشاء المسمى الوظيفي");
    },
  });
}
